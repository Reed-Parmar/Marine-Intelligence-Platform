"""
FastAPI APIRouter for Fisheries Catch Prediction V2.
Designed for seamless, single-line plug-and-play mounting into the unified Marine Intelligence Platform:
    from integration_package.api.catch_routes_v2 import router as fisheries_v2_router
    app.include_router(fisheries_v2_router)
"""
import os
import sys
...
from inference.predict_v2 import FisheriesCatchPredictorV2


router = APIRouter(prefix="/fisheries-v2", tags=["Fisheries Catch Prediction V2"])

MODEL_DIR = os.path.join(pkg_root, "model")
PREDICTOR_V2: Optional[FisheriesCatchPredictorV2] = None


def get_predictor_v2() -> FisheriesCatchPredictorV2:
...
@router.get("/health")
def fisheries_v2_health():
    """Health check endpoint for V2 fisheries predictor."""
    pred = get_predictor_v2()
    return {
        "status": "healthy",
        "component": "Fisheries Catch Prediction V2 (Phase 14.3 V2)",
...
@router.post("/predict", response_model=CatchPredictionResponseV2)
def predict_single_catch(request: FisheriesCatchRequestV2):
...
@router.post("/predict/batch", response_model=BatchCatchResponseV2)
def predict_batch_catch(request: BatchCatchRequestV2):
